...
The old _LOW_VALUE_MARKERS scored every one of those at 0.0 penalty, and a soft
penalty could not have saved it anyway: it caps at 2.3 against interest scores
above 11.
...
Matching is boundary-anchored for the same reason as news.interest: a raw
substring test makes "always to" trip the "ways to" listicle rule and quietly
drops a real story. A trailing `*` marks a stem.
"""
...
_REVIEW = (
    "review:", "review", "hands-on", "hands on with", "unboxing",
...
_NUMBERED_LIST = re.compile(r"^\s*(?:the\s+)?\d{1,2}\s+\w+", re.I)
...
def reject_reason(title: str, summary: str = "") -> str | None:
    ...
    for name, rx in _COMPILED:
        ...
        if match:
            return f"{name}:{match.group(0).strip().lower()}"
    # A four-digit leading number is a year ("2026 was the hottest on record"),
    # not a list count.
