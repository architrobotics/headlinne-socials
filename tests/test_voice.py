...
# Track assembly
# --------------------------------------------------------------------------- #
def test_the_whole_reel_is_narrated_in_one_request():
    stub = StubVoice()
    with tempfile.TemporaryDirectory() as tmp:
        track = build_voice_track(_reel(), Path(tmp) / "v.wav", client=stub)
    assert track is not None
    # Quota is counted per minute, so a reel is one request rather than one per
    # beat: seven calls spent longer being paced than the reel lasts.
    assert len(stub.calls) == 1
    assert len(track.beat_seconds) == 4


def test_every_beat_and_the_sign_off_are_in_the_script():
    stub = StubVoice()
    with tempfile.TemporaryDirectory() as tmp:
        build_voice_track(_reel(), Path(tmp) / "v.wav", client=stub)
    script = stub.calls[0]
    text = script if isinstance(script, str) else script[0]
    assert text.count("\n\n") == 4     # four beats and the sign-off


def test_the_beats_share_out_the_time_that_was_actually_spoken():
    # One request comes back as one blob, so each beat takes the share its line
    # earns. The total is what has to be right; the split only has to be fair.
    stub = StubVoice(seconds=12.0)
    with tempfile.TemporaryDirectory() as tmp:
        track = build_voice_track(_reel(), Path(tmp) / "v.wav", client=stub)
    assert abs(sum(track.beat_seconds) + track.outro_seconds - 12.0) < 1.0
    assert all(s > 0 for s in track.beat_seconds)
...
def test_the_spoken_line_falls_back_to_the_on_screen_text():  # noqa: D103
    stub = StubVoice()
    reel = _reel()
    reel.beats[1].narration = ""
    with tempfile.TemporaryDirectory() as tmp:
        build_voice_track(reel, Path(tmp) / "v.wav", client=stub)
    spoken = stub.calls[0][0]
...
# Failure
# --------------------------------------------------------------------------- #
def test_a_failed_request_abandons_the_whole_track():
    # Partial narration reads as broken, and there is only one request to fail.
    stub = StubVoice(fail_on=1)
    with tempfile.TemporaryDirectory() as tmp:
        assert build_voice_track(_reel(), Path(tmp) / "v.wav", client=stub) is None


def test_a_beat_with_nothing_to_say_abandons_the_track_before_calling_out():
    reel = _reel()
    reel.beats[1].caption = ""
    reel.beats[1].narration = ""
    reel.beats[1].detail = ""
    stub = StubVoice()
    with tempfile.TemporaryDirectory() as tmp:
        assert build_voice_track(reel, Path(tmp) / "v.wav", client=stub) is None
    assert stub.calls == []
